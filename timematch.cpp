...
namespace TIME
{
	Matrix< double, 7, 7> RotationMatrix; //旋转矩阵
	Matrix< double, 7, 7> anzhiMatrix; //安置矩阵
	Matrix<double, 7, 7> Rw; //Rw矩阵
	...
	{
		list<Matrix<double, 7, Dynamic>>::iterator lidarpin = lidar.begin();
		int cols = imu.cols();
		int i = 0;//循环到第几个
		list<Matrix<double, 7, Dynamic>>::iterator temp;
		list<Matrix<double, 7, Dynamic>> listtemp;
	...
	void time_match_m_l_txt(MatrixXd& imu, list<Matrix<double, 7, Dynamic>>& lidar, double CarrierToLidar_x, double CarrierToLidar_y, double CarrierToLidar_z, int number)
	{
		...
		outFile << "timetamp  original_x  original_y  original_z  number  intensity  distance  Lat Lon  Alti  h  p  r  result_x  result_y  result_z\n";
		list<Matrix<double, 7, Dynamic>>::iterator lidarpin = lidar.begin();
		int cols = imu.cols();
		int i = 0;//循环到第几个
		list<Matrix<double, 7, Dynamic>>::iterator temp;
		...
		while (lidarpin != lidar.end() && i < cols)
		{
			...
			if (fabs((*lidarpin)(0, 0) - imu(0, i)) < 1e-3)
			{
				...
				Matrix<double, 7, Dynamic> temp_lm = (*lidarpin).block(0, 0, (*lidarpin).rows(), (*lidarpin).cols());
				Act((*lidarpin), imu(1, i), imu(2, i), imu(3, i), imu(4, i), imu(5, i), imu(6, i), CarrierToLidar_x, CarrierToLidar_y, CarrierToLidar_z);
				for (int j = 0; j < (*lidarpin).cols(); j++)
				{
					outFile << setprecision(14) << (*lidarpin)(0, j) << "\t" << temp_lm(1, j) << "\t" << temp_lm(2, j) << "\t" << temp_lm(3, j) << "\t" << (*lidarpin)(5, j) << "\t" << (*lidarpin)(4, j) << "\t" << (*lidarpin)(6, j) << "\t" << imu(1, i) << "\t" << imu(2, i) << "\t" << imu(3, i) << "\t" << imu(6, i) << "\t" << imu(5, i) << "\t" << imu(4, i) << "\t" << (*lidarpin)(1, j) << "\t" << (*lidarpin)(2, j) << "\t" << (*lidarpin)(3, j) << '\n';
				}
				lidarpin++;
			}
			else if ((*lidarpin)(0, 0) > imu(0, i))
	...
	void Act(Matrix<double, 7, Dynamic >& result_spilt, double xx, double yy, double zz, double r, double p, double h, double CarrierToLidar_x, double CarrierToLidar_y, double CarrierToLidar_z)
	{
		...
		for (int i = 0; i < result_spilt.cols(); i++)
		{
			...
			if (from_latlon(W_GPS[0], W_GPS[1], GPSeast, GPSnorth, zone_number, zone_letter))
			{
				...
				/*
				if ((result_spilt(1, i) < 0) || (result_spilt(2, i) < 0) || (result_spilt(3, i) < 0))
				{
					printf("出现了\n");
				}
				*/
	...
	void Parallel(Matrix<double, 7, Dynamic >& result_spilt, double CarrierToLidar_x, double CarrierToLidar_y, double CarrierToLidar_z)     //矩阵平移
	{
		...
		for (int i = 0; i < t; i++)
		{
			...
			result_spilt(1, i) = CarrierToLidar_x + line3;
			result_spilt(2, i) = CarrierToLidar_y + line1;
			result_spilt(3, i) = CarrierToLidar_z + line2;
		}
	}
